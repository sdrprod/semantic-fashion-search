 ...
 * Always frames output as women's fashion to prevent men's product contamination
 */
async function analyzeImagesWithVision(imageDataUrls: string[]): Promise<string> {
  try {
    ...
    const content: any[] = [
      {
        type: 'text',
        text: 'Describe these women\'s fashion items concisely: type, style, colors, patterns, materials, fit. Focus on the single most important outfit or item. Frame everything as women\'s clothing.'
      }
    ];
  ...
  try {
    ...
    if (files.length > 0) {
      ...
      try {
        ...
        // Always prefix with "women's" to prevent men's product contamination
        if (textQuery && textQuery.trim().length > 0) {
          // Hybrid search: combine user's text with AI-generated image description
          finalSearchQuery = `women's fashion: ${textQuery.trim()}. Style details from images: ${imageDescription}`;
          console.log('[Visual Search API] Hybrid search query:', finalSearchQuery);
        } else {
          // Image-only search
          finalSearchQuery = `women's fashion: ${imageDescription}`;
    ...
    const searchResponse = await semanticSearch(finalSearchQuery, {
      ...
      skipVisionReranking: true, // Query already generated by vision analysis - skip double processing
    });
