-- ============================================
-- Fix Product Feedback Table
-- Run this in Supabase SQL Editor
-- ============================================

-- Option 1: Drop and recreate (safe for development, loses any existing data)
DROP TABLE IF EXISTS public.product_feedback CASCADE;

-- Create the feedback table with all required columns
...
CREATE INDEX idx_feedback_product ON public.product_feedback(product_id);
CREATE INDEX idx_feedback_created ON public.product_feedback(created_at DESC);
CREATE INDEX idx_feedback_session ON public.product_feedback(session_id);
CREATE INDEX idx_feedback_user ON public.product_feedback(user_id) WHERE user_id IS NOT NULL;
...
DROP POLICY IF EXISTS "Public can manage feedback" ON public.product_feedback;
